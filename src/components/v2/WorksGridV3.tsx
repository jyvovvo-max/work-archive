...
function TitleRow({
  ...
  const handleMouseEnter = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Snap to the grid column where code ends
  ...
  return (
    ...
    >
      ...
      {/* Hover image — fixed position, 1 grid column wide */}
      <AnimatePresence>
...
// ── Main ──
interface WorksGridV3Props {
  projects: Project[];
  onOpen: (p: Project) => void;
}

export default function WorksGridV3({ projects, onOpen }: WorksGridV3Props) {
  ...
  // Measure longest title and fit to single line on mobile
  const longestTitle = useMemo(() => {
    if (projects.length === 0) return "";
    return projects.reduce((a, b) => a.title.length > b.title.length ? a : b).title;
  }, [projects]);

  const calcMobileFont = useCallback(() => {
    if (!isMobile || !measureRef.current) return;
    const span = measureRef.current;
    const containerW = window.innerWidth - 40; // minus GUTTER both sides (~20px each)
    const codeW = 80; // approximate code label width + margin
  ...
  return (
    <section style={{ background: "#F0F0F0", position: "relative" }}>
      {/* Hidden measurement span for longest title */}
      {isMobile && (
        <span
          ref={measureRef}
          aria-hidden
      ...
      <div style={{
        ...
        marginBottom: isMobile ? "24px" : "clamp(32px, 4vw, 56px)",
        position: "relative",
        zIndex: 1,
      }}>
        <span style={{
