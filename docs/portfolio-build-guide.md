...
**널싱퀴즈 — 공중보건학개론 5장 19절 570문제 전체 재작성 + 4차원 병렬 검증·패치**

- **위치:** `c:\Users\jyvov\.gemini\antigravity\scratch\nursing-quiz` (포트폴리오와 별개 프로젝트)
- **배경:** 기존 PB_RAW(공중보건학) 19절·570문제가 책 *"최신 공중보건학 개론"(연세대 간호대 이정열·은하출판사)* 의 위계는 맞지만 본문 정확도·문제 품질이 떨어진다는 사용자 보고. 책 표지/목차 사진만 제공받아 표준 보건간호조무사 시험 지식 기반으로 절당 30문제 전부 재작성 결정.
- **5개 병렬 에이전트로 장별 재작성:** 1장(2절 60문제)·2장(6절 180문제)·3장(4절 120문제)·4장(2절 60문제)·5장(5절 150문제) → `c:/tmp/pb-ch{1-5}-v2.json`
- **정답 인덱스 분포 자동 재배치(신규 패턴):** 일부 에이전트(특히 5장)가 정답을 0번에 쏠리게 작성. Node 스크립트로 패턴 `[i%4]` 기반 결정적 매핑 후 보기 swap으로 19개 절 모두 분포 7/8/7/8로 균등화. PB_RAW 머지는 라인 4217-4883 영역 직접 교체 + `new Function()`으로 JS 평가 무결성 검증.
- **4개 병렬 검증 에이전트(신규 패턴):** 각각 한 가지 차원만 특화 — V1 사실 오류, V2 절 학습범위 일치, V3 선택지 품질(길이 편차·카테고리·함정 부재·키워드 매칭·극단어 단서), V4 오탈자·문법. 보고서를 `c:/tmp/verify-v{1-4}.md`로 분리 출력해 우선순위 패치 가능.
- **우선순위 패치 27건 적용:**
  - 사실 오류 5건 (절04-Q5 디프테리아 분류 해설 정정 — "2급" → "1급", 절08-Q8 5대/6대 암 일관성, 절04-Q29 4급 신고 표현, 절13-Q6 영유아 중복 표현, 절14-Q22 BCG+B형간염 묶음 분리)
  - 오탈자 14건 (챠드윅→채드윅 7건, 전국민→전 국민 2건, 치료중심→치료 중심, 자녀가치관→자녀 가치관, 비문 "보건관리자가 두도록"→"를", 「국민건강증진법」 부착 2건)
  - 선택지 품질 8건 (절17-Q21 "수술 도구"→"처방전 발급기", 절15-Q4 오답 극단어 "강제·만·해서만" 3건, 절16-Q25 오답 "~만 가진 자" 3건 자연화)
- **남은 권장사항(보고만):** V2 절 간 중복 약 12건(절09↔절10 인구피라미드 5유형, 절11 출산력 저하 원인 3중 반복 등), V3 길이 편차 약 30건, V4 띄어쓰기 정책 통일(건강증진/건강 증진, 보건교육/보건 교육 등).
- **배포 URL:** https://nursing-quiz-lake.vercel.app — alias 자동 적용
- **백업:** `index.html.pre-pbv2.bak`
...
**히어로 글로브 시안(test-hero-d) 완성 + 상세 페이지 개선 — 프로덕션 배포 (커밋 158e3a7)**

- **작업 도구 전환:** 안티그래비티 → Claude Code. 모델 티어 오케스트레이션 확립 (쉬운 일=Sonnet, 중간=Opus 4.8, 오케스트레이션·고급=Fable 5), `docs/CODEMAP.md` 신설로 섹터별 파인포인트 접근.
- **신규 히어로 시안 `/test-hero-d`** (메인 `/` 미교체, 시안 URL로만 라이브):
  - 카메라가 구 안에 있는 3D 글로브 — 링 3줄×22장, 100% CSS 키프레임 공전(프레임당 JS 0), 반(半)곡률 보정(공전 주기 2배 레이어, face=270°−θ/2), 글로벌 리듬 펄스(9s/12°), 랜딩 스윙 등장(−75°→0), Cloudinary `w_800` 경량 텍스처
  - 호버: 공전 정지 + CRT 브라운관 파동(SVG feDisplacementMap, primitiveUnits=objectBoundingBox 필수 — 아니면 이중 레이어 현상) + 젤리 스쿼시. 클릭 시 파동 후 상세 전환
  - 스크롤: 타이틀 래치(소개문이 타이틀 아래 도킹 후 함께 퇴장, 런타임 측정), 구가 카메라 관통 플라이스루, 트랙 단축(FLY×0.72)으로 블랙 섹션과 겹침 전환
  - Selected Works 횡스크롤 쇼케이스: 블랙 배경, 스냅 매핑(dwell 35%/이동 30%, 슬라이드당 85vh), 거터 정렬 픽셀 지오메트리, 우측 국/영문 설명 컬럼(공간 부족 시 하단 폴백), 대기 슬라이드 blur 7px
  - **함정 노트:** 카메라가 구 안이면 스테이지/컨테이너 투명 판이 히트테스트를 가로챔 → `pointerEvents:none` + 카드만 auto. framer `useScroll` target ref는 빈 상태 반환에도 DOM에 붙어 있어야 함.
- **상세 페이지(ProjectDetailV2):** 라벨 `#NNN-Mon-YYYY`, prev/next 80% 축소, 추출 배경색 채도 ×1.1, 타이틀·본문 105%, 갤러리 가로/세로 간격 통일(`clamp(6.4px,0.96vw,12.8px)`)
- **버그픽스:** Turbopack 워크스페이스 루트 오인식 → `next.config.ts`에 `turbopack.root` 고정 (tailwind 해석 실패 + os error 1450 연쇄 해결)
- **배포:** GitHub push는 자동 배포 미연동 — **이 프로젝트는 `npx vercel --prod --yes` CLI 배포 필수**. git push 인증은 `gh auth git-credential` 헬퍼로 해결.
- **다음 세션:** 모바일 대응 — 데스크톱과 다른 접근 필요 (글로브·횡스크롤 모두 터치/세로 화면 기준 재설계 검토). dev 서버는 3100 포트(3000은 bd2-hub 점유).

---

*이 문서는 Deep Field 포트폴리오 작업 기록입니다. 2026-07-07 기준 최종 수정.*
